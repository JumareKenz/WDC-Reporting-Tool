...
def validate_report_month(submitted_month: str) -> Tuple[bool, str]:
    """
    Validate that the submitted report month is a valid past or current month.
    ...
    """
    try:
        submitted_date = datetime.strptime(submitted_month, '%Y-%m')
    except ValueError:
        return False, f"Invalid month format: {submitted_month}. Expected YYYY-MM."

    now = datetime.utcnow()
    # Compare year-month only (set both to day 1)
    current_month_start = datetime(now.year, now.month, 1)

    if submitted_date > current_month_start:
        return False, f"Cannot submit a report for a future month ({submitted_month}). The month has not yet ended."
