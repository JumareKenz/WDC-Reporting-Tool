...
from ..schemas import (
    ...
    CheckSubmittedResponse, BatchCheckSubmittedResponse, VoiceNoteSimple, WardSimple, UserSimple,
)
...
from ..utils.date_utils import validate_report_month, get_month_display_info, format_month_display
...
async def create_report(
    ...
    report_kwargs = {
        ...
        'meetings_held': 1,  # 1 meeting per month = 1 report
...
@router.put("/{report_id:int}", response_model=ReportResponse)
async def update_report(
    ...
    existing_report.meetings_held = 1  # 1 meeting per month = 1 report
...
def get_reports(
    ...
    """Get all reports for current user's ward (including SUBMITTED and REVIEWED)."""
    ...
    for report in reports:
        ...
        result.append({
            "id": int(report.id) if report.id else None,
            "report_month": str(report.report_month) if report.report_month else None,
            "meetings_held": int(report.meetings_held) if report.meetings_held else 0,
            "attendees_count": int(report.attendees_count) if report.attendees_count else 0,
            "status": str(report.status) if report.status else None,
            "submitted_at": report.submitted_at.isoformat() if report.submitted_at else None,
            "has_voice_note": bool(has_voice_note)
        })

    return result


@router.get("/check-submitted", response_model=None)
def check_submitted(
    month: Optional[str] = None,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Check if current month's report has been submitted for the user's ward."""
    ...
    print(f"DEBUG check_submitted: user_id={current_user.id}, ward_id={current_user.ward_id}, month={month}")

    # Check for any report in this ward/month (not just by current user)
    report = (
        db.query(Report)
        .filter(
            Report.ward_id == current_user.ward_id,
    ...
    print(f"DEBUG check_submitted: found report={report is not None}")

    if report:
        return {
            "month": str(month),
            "submitted": True,
            "report_id": int(report.id) if report.id else None,
            "submitted_at": report.submitted_at.isoformat() if report.submitted_at else None,
        }
...
@router.get("/my-submissions", response_model=None)
def get_my_submissions(
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    """Get all submitted months and reports for the current WDC Secretary."""
    ...
    # Debug logging
    print(f"DEBUG get_my_submissions: user_id={current_user.id}, ward_id={current_user.ward_id}")

    # Get all reports for this user's ward
    reports = db.query(Report).filter(
        Report.ward_id == current_user.ward_id,
        Report.status.in_(["SUBMITTED", "REVIEWED"])
    ).order_by(Report.report_month.desc()).all()

    print(f"DEBUG get_my_submissions: found {len(reports)} reports for ward_id={current_user.ward_id}")

    # Extract submitted months
    submitted_months = [r.report_month for r in reports]

    # Build report list items - return as plain dict with explicit type casting
    report_items = []
    for report in reports:
        has_voice_note = db.query(VoiceNote).filter(VoiceNote.report_id == report.id).first() is not None
        report_items.append({
...
def get_submission_info(
    ...
    Optional query param:
        report_month: specific month to check (YYYY-MM). If not provided, uses auto-detected target month.
    """
    info = get_month_display_info()
    target_month = report_month if report_month else info['target_month']

    # Resolve display name for the requested month
    if report_month:
        target_month_name = format_month_display(report_month)
    else:
        target_month_name = info['month_name']
    ...
    if current_user.role == "WDC_SECRETARY" and current_user.ward_id:
        ...
        existing_report = db.query(Report).filter(
            Report.ward_id == current_user.ward_id,
            Report.report_month == target_month,
            Report.status.in_(["SUBMITTED", "REVIEWED"])
        ).first()
...
@router.get("/{report_id:int}", response_model=ReportResponse)
def get_report(
    report_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
...
@router.patch("/{report_id:int}/review")
def review_report(
    report_id: int,
...
@router.delete("/draft/{draft_id:int}")
def delete_draft(
    draft_id: int,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
