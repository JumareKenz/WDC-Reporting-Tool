...
export const API_ENDPOINTS = {
  ...
  // App version (used by useAppVersion hook for native update checks)
...
export const DEMO_CREDENTIALS = [
  {
    ...
    name: 'Deborah Yohanna Ishaku (WDC Secretary - Chikun)',
  },
  {
    email: 'coord.chk@kaduna.gov.ng',
    password: 'demo123',
    role: USER_ROLES.LGA_COORDINATOR,
    name: 'Ibrahim Garba Danjuma (LGA Coordinator - Chikun)',
  },
  {
    email: 'state.admin@kaduna.gov.ng',
    password: 'demo123',
    role: USER_ROLES.STATE_OFFICIAL,
    name: 'Abdulrazak Mukhtar (State Official)',
  },
];
...
// Storage Keys
// NOTE: AUTH_TOKEN and USER_DATA are legacy localStorage keys retained only for
// the one-time migration (migrateLegacyLocalStorage.js).  New code must not
// write auth tokens to localStorage — use secureStorage from plugins/capacitor.js.
export const STORAGE_KEYS = {
  // Legacy web keys (kept for migration; do not use for new token writes)
  AUTH_TOKEN: 'token',
  USER_DATA:  'user',
  // Capacitor-aware keys used by useAuth and useOfflineQueue
