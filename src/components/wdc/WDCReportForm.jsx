 ...
 */
const WDCReportForm = ({ onSuccess, onCancel, userWard, userLGA, submissionInfo, submissionMethod = 'wizard' }) => {
  const { user } = useAuth();
  ...
  const submitMutation = useMutation({
    mutationFn: async (data) => {
      ...
      formPayload.append('formVersionId', formVersionId);
      formPayload.append('submissionMethod', submissionMethod);
      if (userWard?.id) formPayload.append('wardId', String(userWard.id));
  ...
  const draftMutation = useMutation({
    mutationFn: async (data) => {
      const formVersionId = getCurrentFormVersionId();
      const formPayload = new FormData();
      if (formVersionId) formPayload.append('formVersionId', formVersionId);
      formPayload.append('submissionMethod', submissionMethod);
      if (userWard?.id) formPayload.append('wardId', String(userWard.id));
      formPayload.append('report_month', reportMonth);
      formPayload.append('report_data', JSON.stringify(data));
  ...
  const { addToQueue, syncQueue, getQueueStats, retryFailed, clearFailed, isSyncing, queue } = useOfflineQueue({
    submitFn: async (data, month, headers) => {
      const formVersionId = getCurrentFormVersionId();
      if (!formVersionId) {
        throw new Error('Form configuration not loaded — cannot submit. Open the app online once to sync.');
      }
