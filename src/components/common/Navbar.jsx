import { Bell, User, LogOut, Menu, Settings, Sun, Moon } from 'lucide-react';
    ...
    <nav className="glass-nav sticky top-0 z-40">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Right: Dark toggle + Notifications + User Menu */}
          <div className="flex items-center gap-1">
            {/* Dark / Light mode toggle */}
            <button
              onClick={toggleDark}
              className="p-2.5 rounded-xl hover:bg-neutral-100 dark:hover:bg-neutral-700 transition-all duration-200 group"
              aria-label={isDark ? 'Switch to light mode' : 'Switch to dark mode'}
            >
              {isDark
                ? <Sun  className="w-5 h-5 text-amber-400 group-hover:text-amber-500 transition-colors" />
                : <Moon className="w-5 h-5 text-neutral-500 group-hover:text-primary-600 transition-colors" />}
            </button>
